  ...
  return [
    {
      ...
      provider: {
        ...
        description:
          'PortalKubo-operated provider profile for public POT-ready API tools.',
    ...
    },
    {
      ...
      fullName: process.env.SEED_PROVIDER_NAME_1 ?? 'Portaldot Data Works',
      username: process.env.SEED_PROVIDER_USERNAME_1 ?? 'portaldot-data-works',
      email: process.env.SEED_PROVIDER_EMAIL_1 ?? 'data@portalkubo.local',
      plan: 'base',
      provider: {
        slug: process.env.SEED_PROVIDER_USERNAME_1 ?? 'portaldot-data-works',
        description:
          'Demo provider profile for POT-settled public data and research APIs.',
        websiteUrl: 'https://portaldot.world'
      }
    },
    {
      walletAddress: requireWallet(
      ...
      username: process.env.SEED_PROVIDER_USERNAME_2 ?? 'supernormal-commerce',
      email: process.env.SEED_PROVIDER_EMAIL_2 ?? 'commerce@portalkubo.local',
      plan: 'free',
      provider: {
        slug: process.env.SEED_PROVIDER_USERNAME_2 ?? 'supernormal-commerce',
        description:
          'Demo provider profile for POT-powered commerce, checkout, and agent payment APIs.',
...
function requireWallet(value: string | undefined, envName: string) {
  ...
  if (!wallet || !isSubstrateAddress(wallet)) {
    throw new Error(`${envName} must be a valid Portaldot wallet address.`)
  }
