...
const deploymentSteps = [
  'Set NEXT_PUBLIC_APP_URL, NEXT_PUBLIC_CONVEX_URL, and Portaldot RPC values in the root environment.',
  'Set NEXT_PUBLIC_PORTALDOT_NETWORK and the deployed ink! contract addresses for POT-paid API calls.',
  'Set AGENT_ATTESTER_PRIVATE_KEY and NEXT_PUBLIC_AGENT_ATTESTOR_ADDRESS for production agent proof anchoring.',
  'Run pnpm typecheck and pnpm build before deployment.',
  'Deploy the Next.js app with the same Portaldot chain and Portaldot payment values used for the verified local build.',
  'Confirm /api/health, /api/openapi.json, /api/reference, /agents, /proofs/[proofId], and an unpaid Portaldot payment product call return expected responses.'
]
      ...
      <section className='bg-panel-sheen border-foreground/10 rounded-lg border p-6'>
        ...
        <div className='mt-4 max-w-3xl space-y-3'>
          ...
          <p className='text-foreground/70 text-sm leading-6'>
            Monitor deployment readiness, Portaldot payment health, autonomous
              ...
              >
                <div className='flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between'>
                  ...
                  <span
                    className={
                      item.state === 'ready'
                        ? 'bg-secondary/15 text-primary dark:text-secondary rounded-full px-2.5 py-1 text-xs font-semibold'
                        : 'bg-warning/15 text-warning-foreground dark:text-warning rounded-full px-2.5 py-1 text-xs font-semibold'
                    }
                  >
                ...
                <p
                  className={
                    item.state === 'ready'
                      ? 'text-primary dark:text-secondary mt-3 font-mono text-sm break-all'
                      : 'text-warning-foreground dark:text-warning mt-3 font-mono text-sm break-all'
                  }
                >
                  {item.value}
                </p>
