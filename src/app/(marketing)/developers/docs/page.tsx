...
const providerDocsSections: DocsSection[] = [
  {
    ...
    body: `
The gateway lets a provider list an existing HTTPS API as a paid marketplace product. Buyers, applications, CLIs, and autonomous agents call the hosted gateway endpoint. the gateway handles the Portaldot payment requirement, POT settlement on Portaldot, provider request forwarding, receipts, and proof metadata.
...
| gateway | Prices the request, collects Portaldot payment, calls the provider, stores receipts, and exposes buyer-facing status. |
| Buyer or agent | Calls the gateway, pays through Portaldot payment or managed credits, then receives the provider result or a pollable job. |
`
  },
  {
    id: 'publishing-flow',
...
4. the gateway pays Portaldot payment/POT with the configured agent signer in production mode.
5. the gateway stores receipts, response hashes, and final deliverables.
6. the gateway writes the Portaldot proof when the run is attested.
...
\`\`\`txt
https://yourdomain.com/api/credits/products/{slug}/call
\`\`\`
...
`
  },
  {
    ...
    body: `
Pricing decides how the gateway calculates the Portaldot payment requirement.
...
Credit-metered products should expose a cheap quote endpoint or a deterministic request field. the gateway converts that number into POT with your configured rate and multiplier.
`
  },
  {
...
`
  },
  {
    id: 'field-priceUsd',
    group: 'Pricing',
    title: 'Price in POT or fallback price',
    body: `
For fixed pricing, this is the exact POT price per paid call. For credit-metered pricing, this is the fallback price if quote calculation is unavailable.

**Input:** numeric POT amount such as \`0.08\`, \`1\`, or \`25\`.  
...
`
  },
  {
    ...
    group: 'Pricing',
    title: 'POT per credit',
    body: `
Conversion rate from provider usage units to POT.

Example: if 100 credits should cost 1 POT, enter \`0.01\`.

\`\`\`txt
credits * POT per credit * multiplier = quoted POT
180 * 0.01 * 1.2 = 2.16 POT
\`\`\`
...
`
  },
  {
    ...
    body: `
Optional markup or discount applied after the credit-to-POT conversion.
...
List this as \`pricingModel: "fixed"\`. the gateway charges the configured POT amount when the paid request succeeds.
    ...
    <div className='mx-auto w-full max-w-7xl px-6 py-14'>
      <section className='space-y-4'>
        ...
        <p className='text-foreground/70 max-w-3xl text-sm leading-6'>
          ...
          endpoints for Portaldot payment-paid calls on {paymentNetwork}.
        </p>
