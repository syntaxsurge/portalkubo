...
export async function POST(
  ...
  if (!latestTopUp) {
    return NextResponse.json(
      { error: 'Managed credit balance is empty. Top up with POT first.' },
      { status: 402 }
    )
  }
  ...
  if (providerResult.status === 'failed') {
    await refundManagedCreditDebit({
      ...
      note: 'Provider failed after reservation; reserved POT was returned to the managed credit balance.'
    })
      ...
      ? await settleManagedCreditDebit({
          ...
          note:
            usageDelta.releaseStatus === 'credit_due'
              ? 'Final usage was lower than the reserved quote; unused POT was returned to the managed credit balance.'
              : usageDelta.releaseStatus === 'delta_payment_required'
                ? 'Final usage exceeded the reserved quote; result release requires the remaining POT.'
