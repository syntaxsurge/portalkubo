...
function ManagedCreditsPanelContent({
  ...
  async function recordTopUp(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setAccount(body.account)
      setStatus('POT top-up recorded for managed API-key usage.')
    ...
    <Card className='space-y-5'>
      <div className='flex flex-col justify-between gap-3 lg:flex-row lg:items-start'>
        <div>
          ...
          <p className='text-foreground/65 mt-2 max-w-2xl text-sm leading-6'>
            Portaldot payment remains the Portaldot payment path. Managed
            credits let a team top up POT once, receive a managed-credit API
            key, and debit usage from the saved balance before provider work
            starts.
          </p>
        </div>
      ...
      <div className='grid gap-3 text-sm md:grid-cols-3'>
        ...
        <Metric
          ...
          value={`${account?.balanceUsdc ?? '0.00'} POT`}
        />
        <Metric
          label='Usage debits'
      ...
      >
        <Input
          ...
          placeholder='25.00'
          required
          aria-label='Top-up amount in POT'
        />
        <Input
