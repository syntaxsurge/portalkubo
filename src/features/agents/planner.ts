...
export const AGENT_PLANNER_PROMPT = [
  ...
  'Goal: choose the smallest useful set of paid API tools that can complete the user objective inside the POT budget.',
  'Rules:',
...
export async function buildDeterministicAgentPlan(
  ...
  return {
    ...
    metadata: buildPlannerSummary(run, actions, {
      ...
      budgetStrategy: `Use the highest-ranked relevant tools without exceeding ${run.budgetCapUsdc.toFixed(2)} POT.`,
...
export function buildPlannerSummary(
  ...
  return {
    ...
    budgetInstruction: `Spend no more than ${run.budgetCapUsdc.toFixed(2)} POT across at most ${run.maxPaidActions} paid action(s).`,
    budgetStrategy:
      options?.budgetStrategy ??
      `Run the most relevant tools first and stop before exceeding ${run.budgetCapUsdc.toFixed(2)} POT.`,
    synthesisInstructions:
...
function scoreSignals(objective: string, product: ApiProduct) {
  ...
  if (hasSignal(objective, objectiveSignals.proof)) {
    score += product.isNativePaymentProtected ? 4 : 0
  }
