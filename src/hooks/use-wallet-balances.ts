...
export function useWalletBalances(
  ...
  const refresh = React.useCallback(async () => {
    ...
    const [nativeResult, stablecoinResult] = await Promise.allSettled([
      readNativeBalance(normalizedWallet),
...
async function getApi() {
  apiPromise ??= ApiPromise.create({
    provider: new WsProvider(portaldotConfig.wsUrl)
  })

  return apiPromise
}

async function readNativeBalance(ownerAddress: string) {
  const api = await getApi()
  const account = await api.query.system.account(ownerAddress)
  const accountJson = account.toJSON() as {
    data?: {
      free?: string | number
    }
  }

  return BigInt(accountJson.data?.free ?? 0)
}

async function readStablecoinBalance(_ownerAddress: string) {
  ...
  throw new Error('Stablecoin contract balance reads are pending deployment.')
}

function formatTokenAmount(amount: bigint, decimals: number) {
  return Number(formatUnitsLocal(amount, decimals)).toLocaleString(undefined, {
